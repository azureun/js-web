...
- 자식 노드 확인하기 
    - <code>hasChildNodes()</code> : 특정 노드에 자식 노드가 있는지 확인하는 함수. 자식 노드 있으면 true 없으면 false.
- 자식 노드에 접근하기 
    - <code>childNodes()</code> : 현재 노드의 자식 노드에 접근하는 함수. 요소 노드, 태그와 태그 사이의 줄바꿈도 자식 노드로 인식(빈 텍스트 노드)
- 원하는 위치에 노드 삽입하기 
    - <code>insertBefore()</code> : 부모 노드에 자식 노드를 추가할 때, 기준이 되는 노드 지정 & 그 앞에 자식 노드 추가.
